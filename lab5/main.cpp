...
int main() {
    ...
    cout << "Zdekodowanie + fitness:" << endl;
    for (auto &chromosome: population) {
        auto decoded = decode(chromosome);
        double fit_chromo = fitness_f(chromosome);
        fitnessed_chromosomes.push_back(fit_chromo);

        cout << decoded.first << ", " << decoded.second << " | " << fit_chromo << endl;
    }

    cout << endl << "Krzyzowanie jednopunktowe:" << endl;
    ...
    cout << "Selekcja ruletkowa: " << selection_roulette(fitnessed_chromosomes) << endl;
}